...
const AppPathFolder = path.join(AppPath)
const AppAsar = AppPath
const WindowsUpdater = path.join(AppPath.slice(0, AppPath.indexOf('app.asar')), 'elevate.exe')
...
var Updater = {
    ...
    progress(
      request(
        ...
        function (error, response, body) {
          ...
          var updateFile = AppPathFolder.slice(0, AppPath.indexOf('app.asar')) + UPDATE_FILE
  ...
  mvOrMove: function (child) {
    var updateAsar = AppPathFolder.slice(0, AppPath.indexOf('app.asar')) + UPDATE_FILE
    var appAsar = AppPathFolder.slice(0, AppPath.indexOf('app.asar')) + "app.asar"
    var winArgs = ''
      ...
      try {
        ...
        if (process.platform === 'win32') {
          ...
          try {
            ...
            fs.writeFileSync(
              WindowsUpdater,
              fs.readFileSync(path.join(AppPathFolder.slice(0, AppPathFolder.indexOf('app.asar')), 'app.asar.unpacked/node_modules/electron-asar-hot-updater/updater.exe'))
            )
          } catch (error) {
