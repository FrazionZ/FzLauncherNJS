...
class Settings extends FzPage {
    ...
    constructor(server){
        ...
        $('.config__server_display_size').on('click', function(){
            ...
            if(parseInt(widthDisplay.val()) < parseInt(widthDisplay.attr('min')) || parseInt(heightDisplay.val()) < parseInt(heightDisplay.attr('min')))
                return instance.notyf('error', 'La taille de la fenêtre doit être au minimum à 800x600')
            var sizeFinal = widthDisplay.val()+":"+heightDisplay.val();
            instance.store.set(instance.keyStoreServerOptions("config__server_display_size"), sizeFinal);
