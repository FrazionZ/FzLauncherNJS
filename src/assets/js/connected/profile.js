...
class Profile extends FzPage {
    ...
    constructor(){
        ...
        // Load another skin
        this.gloalSkinViewer.loadSkin(skinUrl, { model: ((this.store.get('session').isSlim) ? "slim" : "default") });
        
        this.gloalSkinViewer.loadCape(capeUrl);
        ...
        $('.updateSkinForm').on('click', function(){
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    var idSkin = $('.skin_item.active').attr("data-skin");
                    var user = instance.store.get('session');
                    user.isSlim = (($('.checkbox.checked').find('input[name="typeSkinInput"]').val() == "alex") ? true : false);
                    instance.store.set('session', user)
                    instance.store.set('selectSkin', idSkin)
                    FZUtils.loadURL('/connected/layout', [{notyf: {type: "success", value: "Votre skin a bien été changé"}}, {session: profile.store.get('session')}, {linkPage: "#profile"}, {openPage: "profile"}])
                },
                error: function(err){
                    console.log(err)
                                    ...
                                    (response) => {
                                        ...
                                        img.onload = function() {
                                            ...
                                            if(imgWidth != 64 || imgHeight != 64){
                                                ...
                                                FZUtils.loadURL('/connected/layout', [{session: profile.store.get('session')}, {linkPage: "#profile"}, {openPage: "profile"}])
                                            }
                                        };
        ...
        this.previewSkinViewer.loadCape(capeUrl);
